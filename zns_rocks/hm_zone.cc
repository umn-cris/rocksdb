...
/*HmZone::HmZone(fstream& fs,size_t id):modify_zone_(fs){
    ...
    if(!modify_zone_.is_open()) cout<<"[hm_zone.cpp] [HmZone] create zone file
failed"<<endl;
...
HmZone::HmZone(std::fstream &fs, size_t id) {
  ...
  if (!modify_zone_.is_open())
    std::cout << "[hm_zone.cpp] [HmZone] create zone file failed" << std::endl;
  // modify_zone_<<ToString()<<endl;
  modify_zone_.close();
  // if(if_debug) cout<<"create zone "<<zoneInfo_.id<<endl;
}
...
Status HmZone::OpenZone() {
  ...
  if(!modify_zone_.is_open()){
      status = Status::NotFound("[hm_zone.cpp] [OpenZone] Open zone failed, zone
...
Status HmZone::ResetWritePointer() {
  ...
  status = Status::OK();
  if (if_debug)
    std::cout << "in [hm_zone] [reset write pointer] " << zoneInfo_.id << std::endl;
  return status;
}
...
Status HmZone::ZoneWrite(ZoneAddress addr, const char *data) {
  ...
  if (zoneInfo_.write_pointer + addr.length > ZONESIZE) {
    status = Status::InvalidArgument(
        "[hm_zone.cpp] [ZoneWrite] WriteZone failed, reach end of zone file, "
        "zone id: " +
  ...
  if (!modify_zone_.is_open()) {
    status = Status::NotFound(
        "[hm_zone.cpp] [ZoneWrite] OpenZone failed, zone id: " +
        std::to_string(zoneInfo_.id));
    return status;
  }
  ...
  if (addr.offset != zoneInfo_.write_pointer) {
    ...
    if (zoneInfo_.zone_type == SEQUENTIAL_WRITE_REQUIRED) {
      std::cout << "" << std::endl;
      status.NotSupported(
          "[hm_zone.cpp] [ZoneWrite] it is not a sequential write, sequential "
  ...
  if (!modify_zone_.good()) {
    if (modify_zone_.fail())
      status = Status::InvalidArgument(
          "[hm_zone.cpp] [ZoneWrite] WriteZone failed, no remain space / write "
          "mode error / format error... zone id: " +
          std::to_string(zoneInfo_.id));
    else
      status = Status::InvalidArgument(
          "[hm_zone.cpp] [ZoneWrite] WriteZone failed, unknown reasons, need "
...
Status HmZone::ZoneRead(ZoneAddress addr, char *data) {
  ...
  if (!modify_zone_.is_open()) {
    status =
        Status::NotFound("[hm_zone.cpp] [ZoneRead] OpenZone failed, zone id: " +
                         std::to_string(zoneInfo_.id));
    return status;
  }
  ...
  if (!modify_zone_.good()) {
    if (modify_zone_.eof())
      status = Status::InvalidArgument(
          "[hm_zone.cpp] [ZoneRead] ReadZone failed, reach end of zone file, "
          "zone id: " +
          std::to_string(zoneInfo_.id));
    else if (modify_zone_.fail())
      status = Status::InvalidArgument(
          "[hm_zone.cpp] [ZoneRead] ReadZone failed, no remain space / read "
          "mode error / format error... zone id: " +
          std::to_string(zoneInfo_.id));
  }
  modify_zone_.close();
...
/*Status HmZoneNamespace::Write(ZoneAddress addr, const char *data) {
    ...
    if(!status.ok()) {
        ...
        status = Status::NotFound("[hm_zone.cpp] [Write] in ZNS write, GetZone
failed, zone id: " + to_string(addr.zone_id)); return status;
    ...
    if(!status.ok()) {
        cout<<status.ToString()<<endl;
        status = Status::NotFound("[hm_zone.cpp] [Write] in ZNS write, WriteZone
failed, zone id: " + to_string(addr.zone_id)); return status;
    }
    return status;
}*/
...
/*Status HmZoneNamespace::Read(ZoneAddress addr,  char *data) {
    ...
    if(!status.ok()) {
        cout<<status.ToString()<<endl;
        status = Status::NotFound("[hm_zone.cpp] [Read] in ZNS read, GetZone
failed, zone id: " + to_string(addr.zone_id)); return status;
    }
    ...
    if(!status.ok()) {
        cout<<status.ToString()<<endl;
        status = Status::NotFound("[hm_zone.cpp] [Read] in ZNS read, ReadZone
failed, zone id: " + to_string(addr.zone_id)); return status;
    }
    return status;
}*/
...
Status HmZoneNamespace::Resetptr(int id) {
  Status status;
  std::shared_ptr<Zone> hmzone = GetZone(id);
  status = hmzone->ResetWritePointer();
  return status;
}
...
Status HmZoneNamespace::NewZone() {
  ...
  if (!it.second) {
    status = Status::InvalidArgument(
        "[hm_zone.cpp] [NewZone] already has a zone in ZNS, zone id: " +
...
// delete all zone files left, create a new zone env (creat zone file, file
// number is an adjustable parameter)
Status HmZoneNamespace::InitZNS(const char *dir_name) {
  ...
  if (!S_ISDIR(s.st_mode)) {
    ...
    if (success == 0) {
      std::cout << "[hm_zone.cpp] [init_scanner] first time open this ZNS, create "
              "dir "
           << dir_name << std::endl;
      status = Status::OK();
    }
    if (success == -1) {
      std::cout << "[hm_zone.cpp] [init_scanner] first time open this ZNS. However, "
              "create dir "
  ...
  if (NULL == dir) {
    std::cout << "[hm_zone.cpp] [init_scanner] Can not open dir " << dir_name
         << std::endl;
...
HmZoneNamespace::HmZoneNamespace() { InitZNS(path.c_str()); }
