...
Status ZnsFileWriterManager::CreateFileByThisWriter(uint64_t now_time,
  ...
  if (found != file_to_writer_.end()) {
    ...
    size_t write_offset;
    return zone_mapping_->CreateFileOnZone(now_time, file_name,
                                           writer->GetZoneID(), &write_offset);
  }
...
Status ZnsFileWriterManager::DeleteFile(uint64_t now_time,
                                        std::string file_name) {
  auto found = file_to_writer_.find(file_name);
  if (found == file_to_writer_.end()) {
...
Status ZnsFileWriterManager::RenameFile(const std::string& from,
                                        const std::string& to) {
...
Status ZnsFileWriterManager::AppendDataOnFile(std::string file_name, size_t len,
                                              const char* buffer) {
  auto found = file_to_writer_.find(file_name);
  if (found == file_to_writer_.end()) {
...
Status ZnsFileWriterManager::ReadDataOnFile(std::string file_name,
...
Status ZnsFileWriterManager::CloseFile(ZnsFileWriter* writer,
                                       std::string file_name) {
  if (writer == nullptr) {
...
ZnsFileWriterManager* GetDefualtZnsFileWriterManager() {
  static ZnsFileWriterManager* zfm_ptr =
      new ZnsFileWriterManager(GetDefaultZoneMapping());
  return zfm_ptr;
}
